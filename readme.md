...
不过关于 BigPipe 的实践还有很大的优化空间，比如说，要 pipe 的内容最好设置一个触发的时间值，如果异步调用的数据很快返回，就不需要用 BigPipe 技术，直接生成网页送出即可，如果数据请求超过一定时间才用 BigPipe，相比 ajax 即节省了浏览器到 node.js 服务器的请求数，又节省了 node.js 服务器到数据源的请求数。具体的优化方法，可能要等到雪球网用上 BigPipe 以后才能分享。顺便打个广告，雪球的技术博客最近开通了，以后雪球的 BigPipe 实践经验分享就会发表在那了，地址是 http://engineering.xueqiu.com/
